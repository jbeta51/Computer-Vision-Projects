...
# generate hist for img
fname = siftdir + fnames[0]
origMat = scipy.io.loadmat(fname)
im = framesdir + str(origMat['imname'][0])
im = plt.imread(im)

plt.imshow(im)
MyROI = roipoly(roicolor='r')
ind = MyROI.getIdx(im, origMat['positions'])

inRegDesc = origMat['descriptors'][ind,:]

hist = origHist(inRegDesc, clf)
